...
HEADER   = ["使用者", "日期時間", "收縮壓", "舒張壓", "心跳", "狀態", "溫度(°C)", "濕度(%)", "生活標籤", "備註"]
...
def load_records(_ws, user_id: str) -> pd.DataFrame:
    """從 Google Sheets 讀取指定使用者的紀錄"""
    rows = _ws.get_all_records()
    if not rows:
        return pd.DataFrame(columns=HEADER)
    df = pd.DataFrame(rows)
    if "使用者" in df.columns:
        df = df[df["使用者"] == user_id]
...
if add_btn:
    ...
    row = [user_id, dt_str, systolic, diastolic, pulse, label_text, cur_temp, cur_humidity, tags_str, note]
    if gs_ok:
        try:
...
if gs_ok:
    try:
        df_for_baseline = load_records(worksheet, user_id)
        for col in ["收縮壓", "舒張壓"]:
...
with chart_col:
    ...
    if gs_ok:
        try:
            df = load_records(worksheet, user_id)
        except Exception:
...
if gs_ok:
    try:
        df_recent = load_records(worksheet, user_id)
    except Exception:
...
if gs_ok:
    try:
        df_history = load_records(worksheet, user_id)
    except Exception as e:
        st.error(f"讀取失敗：{e}")
