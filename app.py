...
血壓健康儀表板
執行方式：
    pip install streamlit pandas plotly requests gspread google-auth
...
.stApp {
    background: linear-gradient(135deg, #0f1923 0%, #1a2a3a 50%, #0d1f30 100%);
    color: #e8f4f8;
}
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0d2137 0%, #112940 100%);
...
#  Google Sheets 連線
# ──────────────────────────────────────────
SHEET_ID = "1mUKhqjTrxl2zTy0vuFpeGDZcaM15qV3Lbsbby3igXiE"
...
def load_records(_ws) -> pd.DataFrame:
    """從 Google Sheets 讀取所有紀錄（ttl=30 秒自動刷新）"""
...
def compute_personal_baseline(df: pd.DataFrame):
    """
    計算個人基礎血壓（前兩週平均與標準差）
    ...
    return {
        "sys_mean": df_base["收縮壓"].mean(),
        "sys_std":  df_base["收縮壓"].std(),
        "dia_mean": df_base["舒張壓"].mean(),
        "dia_std":  df_base["舒張壓"].std(),
        "count":    len(df_base),
    }

def personal_baseline_alert(systolic: int, diastolic: int, baseline: dict):
    """
    依個人基礎血壓判斷是否異常波動
    超過個人平均 + 1.5 個標準差即視為異常
    回傳 (是否異常, 警告訊息列表)
    """
    alerts = []
    is_abnormal = False
    threshold = 1.5  # 幾個標準差視為異常

    if baseline is None:
        return False, []

    sys_upper = baseline["sys_mean"] + threshold * max(baseline["sys_std"], 3)
    dia_upper = baseline["dia_mean"] + threshold * max(baseline["dia_std"], 3)

    if systolic > sys_upper:
        diff = systolic - baseline["sys_mean"]
        alerts.append(
            f"📊 收縮壓 {systolic} mmHg 比您個人兩週基準（{baseline['sys_mean']:.0f} mmHg）"
            f"高出 **{diff:.0f} mmHg**，出現個人異常波動，請留意。"
        )
        is_abnormal = True

    if diastolic > dia_upper:
        diff = diastolic - baseline["dia_mean"]
        alerts.append(
            f"📊 舒張壓 {diastolic} mmHg 比您個人兩週基準（{baseline['dia_mean']:.0f} mmHg）"
            f"高出 **{diff:.0f} mmHg**，出現個人異常波動，請留意。"
        )
        is_abnormal = True
...
def bp_advice(systolic: int, diastolic: int, temp, humidity) -> list:
    """根據血壓、溫度、濕度給出具體建議"""
    advice = []
    ...
    if isinstance(temp, (int, float)):
        if temp <= 18 and (systolic >= 130 or diastolic >= 80):
            advice.append("🧥 氣溫偏低，血管容易收縮使血壓上升，建議開啟暖氣將室內維持在 20–25°C。")
            advice.append("🛁 避免突然進出冷熱環境（如從暖房走到戶外），溫差過大對心血管負擔大。")
            advice.append("🧣 外出時注意保暖，尤其頸部、手腕、腳踝等血管較淺的部位。")
        elif temp >= 30 and (systolic >= 130 or diastolic >= 80):
            advice.append("🌡️ 高溫容易使血管擴張、心跳加速，請避免在正午外出，多補充水分。")

    if isinstance(humidity, (int, float)):
        if humidity >= 80 and (systolic >= 130 or diastolic >= 80):
            advice.append("💧 高濕度悶熱環境增加心臟負擔，建議待在通風或有冷氣的室內。")

    return advice


def ai_attribution_analysis(df: pd.DataFrame, now_temp) -> list[str]:
    """
    交叉比對生活標籤、天氣與血壓異常，找出個人高風險組合
    """
    insights = []
    if df.empty or "生活標籤" not in df.columns:
        return insights
    ...
    for tag, rate, count in sorted(tag_results, key=lambda x: -x[1]):
        insights.append(
            f"🏷️ **{tag}** 出現時，您的血壓超標機率為 **{rate*100:.0f}%**（共 {count} 次紀錄）"
        )
    ...
    if has_temp.sum() >= 3:
        ...
        for tag in ["😴 睡眠不足", "💊 忘記服藥", "😰 壓力較大"]:
            ...
            if combo.sum() >= 2:
                ...
                if rate >= 0.7:
                    insights.append(
                        f"🌡️+🏷️ **氣溫低於 18°C** 且 **{tag}** 同時出現時，"
                        f"您的血壓超標機率高達 **{rate*100:.0f}%**（共 {combo.sum()} 次）"
                    )
    ...
    if len(abnormal_df) >= 2:
        ...
        if all_tags:
            ...
            if top_count >= 2:
                insights.append(
                    f"📌 您血壓超標時最常出現的標籤是 **{top_tag}**（共 {top_count} 次），建議特別留意。"
                )
...
def weather_code_to_desc(code: int) -> str:
    wmo = {
        0:"☀️ 晴天",1:"🌤 大致晴朗",2:"⛅ 局部多雲",3:"☁️ 陰天",
        45:"🌫 霧",48:"🌫 霧凇",51:"🌦 毛毛雨",53:"🌦 中度毛毛雨",
        55:"🌧 濃密毛毛雨",61:"🌧 小雨",63:"🌧 中雨",65:"🌧 大雨",
        71:"🌨 小雪",73:"🌨 中雪",75:"❄️ 大雪",80:"🌦 陣雨",
...
with st.sidebar:
    ...
    if gs_ok:
        st.success("☁️ 已連接 Google Sheets", icon="✅")
    else:
        st.error(f"⚠️ Google Sheets 連線失敗\n\n{gs_error}")

    st.markdown("---")
    record_date = st.date_input("📅 量測日期", value=date.today())
    record_time = st.time_input("⏰ 量測時間", value=datetime.now().time())

    st.markdown("### 血壓數值")
    st.info("💡 建議量測方式：坐下休息 5 分鐘後，每隔 1–2 分鐘量一次，共量 3 次取平均，結果更準確。")
...
if gs_ok:
    try:
        df_for_baseline = load_records(worksheet)
        for col in ["收縮壓", "舒張壓"]:
            df_for_baseline[col] = pd.to_numeric(df_for_baseline[col], errors="coerce")
    except Exception:
        df_for_baseline = pd.DataFrame(columns=HEADER)
else:
    df_for_baseline = pd.DataFrame(columns=HEADER)

baseline = compute_personal_baseline(df_for_baseline)
...
if baseline:
    ...
    # 指標卡片下方顯示個人基準線資訊
    baseline_info = (
        f"🤖 您的個人兩週基準：收縮壓 **{baseline['sys_mean']:.0f}** mmHg｜"
        f"舒張壓 **{baseline['dia_mean']:.0f}** mmHg"
        f"（依 {baseline['count']} 筆紀錄計算）"
    )
else:
...
cards = [("收縮壓", systolic, "mmHg"), ("舒張壓", diastolic, "mmHg"), ("心跳", pulse, "bpm"), ("狀態", label, "")]
for col, (lbl, val, unit) in zip([col1, col2, col3, col4], cards):
    with col:
        if lbl == "狀態":
            st.markdown(f"""
            <div class="metric-card">
                <div class="metric-label">{lbl}</div>
                <div style="margin-top:12px;"><span class="badge {badge_cls}">{val}</span></div>
                <div class="metric-unit" style="margin-top:10px;">依 AHA 準則</div>
...
with weather_col:
    ...
    if weather:
        ...
        if isinstance(pressure, (int, float)) and pressure < 1000:
            ...
            st.markdown('<div class="info-banner">✅ 今日天氣狀況良好，適合戶外活動與量測血壓。</div>', unsafe_allow_html=True)
    else:
        st.warning("⚠️ 無法取得天氣資料，請確認網路連線。")
...
with chart_col:
    ...
    if df.empty:
        demo_data = [
            ...
            {"日期時間":"05/01 20:00","收縮壓":124,"舒張壓":82,"心跳":74},
            {"日期時間":"05/02 08:00","收縮壓":121,"舒張壓":79,"心跳":68},
            {"日期時間":"05/02 20:00","收縮壓":128,"舒張壓":85,"心跳":76},
    ...
    fig = go.Figure()
    fig.add_hrect(y0=90, y1=120, fillcolor="rgba(52,211,153,0.06)", line_width=0,
                  annotation_text="正常收縮壓範圍", annotation_position="top left",
                  annotation_font=dict(size=10, color="#34d399"))
    fig.add_hrect(y0=60, y1=80, fillcolor="rgba(99,179,237,0.06)", line_width=0,
                  annotation_text="正常舒張壓範圍", annotation_position="bottom left",
                  annotation_font=dict(size=10, color="#63b3ed"))
    fig.add_trace(go.Scatter(
        x=df["日期時間"], y=pd.to_numeric(df["收縮壓"], errors="coerce"),
        mode="lines+markers", name="收縮壓",
        line=dict(color="#ef4444", width=2.5),
        marker=dict(size=7, color="#ef4444", line=dict(color="white", width=1.5)),
        hovertemplate="<b>收縮壓</b>: %{y} mmHg<extra></extra>"))
    fig.add_trace(go.Scatter(
        x=df["日期時間"], y=pd.to_numeric(df["舒張壓"], errors="coerce"),
        mode="lines+markers", name="舒張壓",
        line=dict(color="#40c4ff", width=2.5),
        marker=dict(size=7, color="#40c4ff", line=dict(color="white", width=1.5)),
        hovertemplate="<b>舒張壓</b>: %{y} mmHg<extra></extra>"))
    fig.add_trace(go.Scatter(
        x=df["日期時間"], y=pd.to_numeric(df["心跳"], errors="coerce"),
        mode="lines+markers", name="心跳",
        line=dict(color="#fbbf24", width=1.8, dash="dot"),
        marker=dict(size=6, symbol="diamond", color="#fbbf24"),
        yaxis="y2",
        hovertemplate="<b>心跳</b>: %{y} bpm<extra></extra>"))
    fig.update_layout(
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=dict(family="Noto Sans TC", color="#c8e6f5"),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1,
                    bgcolor="rgba(0,0,0,0)", bordercolor="rgba(64,196,255,0.2)", borderwidth=1),
        xaxis=dict(showgrid=True, gridcolor="rgba(64,196,255,0.08)", tickfont=dict(size=11)),
        yaxis=dict(title="血壓 (mmHg)", showgrid=True, gridcolor="rgba(64,196,255,0.08)", range=[50, 180]),
        yaxis2=dict(title=dict(text="心跳 (bpm)", font=dict(color="#fbbf24")),
                    overlaying="y", side="right", showgrid=False, range=[40, 160],
...
if gs_ok:
    try:
        df_recent = load_records(worksheet)
    except Exception:
        df_recent = pd.DataFrame(columns=HEADER)
else:
    df_recent = pd.DataFrame(columns=HEADER)
...
if not df_recent.empty:
    ...
    # 從最後一筆往前找，時間差在 1 小時內的連續紀錄（最多取 3 筆）
    valid_rows = []
    ...
    for i in range(last_idx, max(last_idx - 5, -1), -1):
        if len(valid_rows) == 0:
            ...
            prev_idx = valid_rows[-1]
            dt_diff = abs((df_recent.loc[prev_idx, "_dt"] - df_recent.loc[i, "_dt"]).total_seconds())
            if dt_diff <= 3600:  # 1 小時 = 3600 秒
                valid_rows.append(i)
        if len(valid_rows) == 3:
            break

    df_avg = df_recent.loc[valid_rows[::-1]]  # 時間順排

    if len(df_avg) == 0:
        st.info("尚無紀錄，無法計算平均。")
    elif len(df_avg) == 1:
        st.info("目前 1 小時內只有 1 筆紀錄，建議再量 1–2 次後查看平均，結果更準確。")
        ...
        with r4:
            ...
            <div class="metric-card" style="padding:12px 16px;">
                <div class="metric-label">平均狀態</div>
        ...
        with st.expander("查看納入計算的紀錄"):
            st.dataframe(
                df_avg[["日期時間","收縮壓","舒張壓","心跳","狀態"]],
                use_container_width=True, hide_index=True
            )

        adv = bp_advice(int(avg_sys), int(avg_dia), now_temp, now_humidity)
        if adv:
            with st.expander("💡 依平均值給出的建議"):
                for a in adv:
                    st.markdown(f"- {a}")
else:
...
# ── 歷史紀錄表格 ──────────────────────────
st.markdown("<br>", unsafe_allow_html=True)
st.markdown('<p class="section-title">📋 歷史紀錄（來自 Google Sheets）</p>', unsafe_allow_html=True)

if gs_ok:
    try:
        df_history = load_records(worksheet)
    except Exception as e:
        st.error(f"讀取失敗：{e}")
...
if not df_history.empty:
    ...
    today = datetime.now()
    df_history["_dt"] = df_history["_dt"].apply(
        lambda x: x.replace(year=today.year) if pd.notnull(x) else x
    )
    ...
    # 早晚分類（早上：5–11點，晚上：18–23點）
    df_history["_hour"] = df_history["_dt"].dt.hour
    df_morning = df_history[(df_history["_hour"] >= 5)  & (df_history["_hour"] < 12)]
    df_evening = df_history[(df_history["_hour"] >= 18) & (df_history["_hour"] < 24)]

    # ── 統計卡片 ──
    st.markdown("#### 📊 血壓統計摘要")
    tab1, tab2, tab3 = st.tabs(["📅 今日", "🗓 本週", "🌅 早晚對比"])
    ...
    with tab2:
        if df_week.empty:
            ...
            c1, c2, c3 = st.columns(3)
            c1.metric("本週平均收縮壓", f"{df_week['收縮壓'].mean():.0f} mmHg",
                      delta=f"{df_week['收縮壓'].iloc[-1] - df_week['收縮壓'].mean():.0f}", delta_color="inverse")
            c2.metric("本週平均舒張壓", f"{df_week['舒張壓'].mean():.0f} mmHg",
                      delta=f"{df_week['舒張壓'].iloc[-1] - df_week['舒張壓'].mean():.0f}", delta_color="inverse")
            c3.metric("本週平均心跳",   f"{df_week['心跳'].mean():.0f} bpm",
                      delta=f"{df_week['心跳'].iloc[-1] - df_week['心跳'].mean():.0f}", delta_color="inverse")
            st.caption(f"共 {len(df_week)} 筆紀錄（本週從 {week_start.strftime('%m/%d')} 起算）")
    ...
    with s_col1:
        diff_sys = df_history['收縮壓'].iloc[-1] - df_history['收縮壓'].mean()
        st.metric("整體平均收縮壓", f"{df_history['收縮壓'].mean():.0f} mmHg",
                  delta=f"{diff_sys:.0f}", delta_color="inverse")
    with s_col2:
        diff_dia = df_history['舒張壓'].iloc[-1] - df_history['舒張壓'].mean()
        st.metric("整體平均舒張壓", f"{df_history['舒張壓'].mean():.0f} mmHg",
                  delta=f"{diff_dia:.0f}", delta_color="inverse")
    with s_col3:
    ...
    st.dataframe(df_history[["日期時間","收縮壓","舒張壓","心跳","狀態","溫度(°C)","濕度(%)","備註"]],
                 use_container_width=True, hide_index=True)
...
# ──────────────────────────────────────────
SHEET_ID = "1mUKhqjTrxl2zTy0vuFpeGDZcaM15qV3Lbsbby3igXiE"
KEY_FILE = "/Users/ch/Desktop/醫療健康作業二/blood-preasure-495402-7bb0e28ad7d6.json"
SCOPES   = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
HEADER   = ["日期時間", "收縮壓", "舒張壓", "心跳", "狀態", "溫度(°C)", "濕度(%)", "備註"]
...
with st.sidebar:
    ...
    if add_btn:
        ...
        if cur_weather:
            cur_temp     = cur_weather["current"].get("temperature_2m", "--")
            cur_humidity = cur_weather["current"].get("relative_humidity_2m", "--")
        row = [dt_str, systolic, diastolic, pulse, label_text, cur_temp, cur_humidity, note]
        if gs_ok:
            try:
                worksheet.append_row(row)
                st.cache_data.clear()
